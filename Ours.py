...
def find_top_indexes(values, indexes, top_count):
    result = []
    for i in range(values.size(0)):
        heap = []
        for j in range(values.size(1)):
            value = values[i, j].item()
            index = indexes[j]
        ...
        top_indexes = [index for _, index in sorted(heap, reverse=True)]
        result.append(top_indexes)
...
class OursLayer(nn.Module):
    ...
    def forward(self,Sinput, Rinput, inter_adj,city_adj, province_adj, source_index,
                record, Coeff12=None, Coeff3=None, Coeff4=None):
        ...
        if record:
            ...
            train.Coeff12new = attention_inter # whole attention coeff
...
class Ours(nn.Module):
    ...
    def forward(self, inter_adj,city_adj, province_adj, source_index, record=False, Coeff12=None, Coeff3=None, Coeff4=None):
        ...
        #def forward(self,Sinput, Rinput, inter_adj,city_adj, province_adj, source_index):
        x = torch.cat([att(s_input, r_input, inter_adj, city_adj, province_adj,source_index, record,Coeff12, Coeff3, Coeff4) for att in self.attentions], dim=1)#(N,M*heads)
